...
import re
import subprocess
# Override getstatusoutput to support shell commands (needed for commands with &&)
# The original commands.getstatusoutput from Python 2 used shell=True by default
_original_getstatusoutput = None
try:
...
def getstatusoutput(cmd):
    """Wrapper for subprocess.getstatusoutput that uses shell=True for shell commands"""
	...
	def longest_orf(self,direction,start_codon={"ATG":None}, stop_codon={"TAG":None,"TAA":None,"TGA":None}):
		...
		if direction == "-":
			...
			if self.result[0] == "-":
				...
				strand_direction = "-"
				orf_integrity = self.result[4]
		return return_orf,start_coordinate,end_coordinate,strand_direction,orf_integrity
...
def calculate_potential(fasta,strand,output_orf,outfile):
	...
	if output_orf == 1:
		my_header = ["#ID","transcript_length","orf_start","orf_end","peptide_length","Fickett_score","pI","ORF_integrity","putative_peptide","coding_probability","label"]
	else:
		my_header = ["#ID","transcript_length","orf_start","orf_end","peptide_length","Fickett_score","pI","ORF_integrity","coding_probability","label"]
	...
	for seq in SeqIO.parse(fasta, "fasta"):
		...
		seqRNA = seqRNA.upper()
		seqCDS,start_pos,end_pos,orf_strand,orf_fullness = FindCDS(seqRNA).longest_orf(strand)
		'''seqCDS:longest ORF'''
		...
		if pep_len > 0:
			...
			# end_pos is already 0-based exclusive, convert to 1-based inclusive
			# For Python slicing seq[start:end], end is exclusive, so last base is at end-1 in 0-based
			# In 1-based: end-1 becomes end, so end_pos stays the same for 1-based inclusive representation
		...
		if output_orf == 1:
			output_line = [seqid,len(seqRNA),start_pos,end_pos,pep_len,fickett_score,isoelectric_point,orf_fullness,newseqprot]
		else:
			output_line = [seqid,len(seqRNA),start_pos,end_pos,pep_len,fickett_score,isoelectric_point,orf_fullness, ""]
	...
	# First try to find libsvm binaries in conda environment
	app_svm_scale = None
	app_svm_predict = None
	conda_prefix = os.environ.get('CONDA_PREFIX')
	if conda_prefix:
		bin_dir = os.path.join(conda_prefix, "bin")
		svm_scale_path = os.path.join(bin_dir, "svm-scale")
		svm_predict_path = os.path.join(bin_dir, "svm-predict")
		if os.path.exists(svm_scale_path) and os.path.exists(svm_predict_path):
			app_svm_scale = svm_scale_path
			app_svm_predict = svm_predict_path
	
	# Fallback to original hardcoded paths if not found in conda
	if not app_svm_scale or not app_svm_predict:
	...
	if not os.path.exists(app_svm_scale) or not os.access(app_svm_scale, os.X_OK):
		sys.stderr.write("[ERROR] No executable svm-scale found! Looked at: " + app_svm_scale + "\n")
		return -1
	if not os.path.exists(app_svm_predict) or not os.access(app_svm_predict, os.X_OK):
		sys.stderr.write("[ERROR] No executable svm-predict found! Looked at: " + app_svm_predict + "\n")
		return -1
	
	# Find CPC2 data files - data files are at ../../data/CPC2/ from script location
	data_dir = os.path.join(script_dir, "..", "..", "data", "CPC2")
	cpc2_range = os.path.join(data_dir, "cpc2.range")
	cpc2_model = os.path.join(data_dir, "cpc2.model")
	
	if not os.path.exists(cpc2_range):
		sys.stderr.write("[ERROR] CPC2 data file not found: " + cpc2_range + "\n")
		return -1
	if not os.path.exists(cpc2_model):
		sys.stderr.write("[ERROR] CPC2 model file not found: " + cpc2_model + "\n")
		return -1
	...
	# Check if command was successful before trying to read output
	if exitstatus != 0:
		sys.stderr.write("[ERROR] SVM prediction command failed with exit status: " + str(exitstatus) + "\n")
		if outtext:
			sys.stderr.write("[ERROR] Command output: " + outtext + "\n")
		return -1
	
	# Check if output file was created
	if not os.path.exists(outfile + '.tmp.out'):
		sys.stderr.write("[ERROR] SVM prediction output file not found: " + outfile + '.tmp.out' + "\n")
		sys.stderr.write("[ERROR] Command may have failed. Exit status: " + str(exitstatus) + "\n")
		if outtext:
			sys.stderr.write("[ERROR] Command output: " + outtext + "\n")
		return -1
