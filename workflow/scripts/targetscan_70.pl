...
sub getUsage
{
	...
	USAGE:
		$0 miRNA_file UTR_file PredictedTargetsOutputFile [-f]
	...
	Optional:
		-f           => Force overwrite of output file without prompting
...
sub checkArguments
{
	...
	{
		if ($force) {
			unlink $coordsFile;  # Delete existing file if force flag is set
		} else {
